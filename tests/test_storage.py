...
def test_get_returns_ticket(tmp_path: Path) -> None:
    store = TicketStore(tmp_path / "tickets")
    created = make_ticket("SEC-1", title="CORS wildcard", category="security", repo="owner/repo")
    store._create(created)

    loaded = store.get("SEC-1")
...
def test_load_ticket_without_due_date_is_none(tmp_path: Path) -> None:
    ...
    store = TicketStore(tmp_path / "tickets")
    ticket = make_ticket("CODE-1", title="No due date")
    store._create(ticket)

    loaded = store.get("CODE-1")
...
def test_get_not_found(tmp_path: Path) -> None:
    ...
    with pytest.raises(TicketNotFoundError, match="Ticket Z-99 not found"):
        store.get("Z-99")


def test_list_filters(tmp_path: Path) -> None:
    store = TicketStore(tmp_path / "tickets")
    code_ticket = make_ticket("CODE-1", title="Code cleanup", category="code_quality")
    security_ticket = make_ticket("SEC-1", title="Fix TLS", category="security")
    store._create(code_ticket)
    store._create(security_ticket)

    results = store.list(SearchFilters(category=["security"]))

    assert [ticket.id for ticket in results] == ["SEC-1"]


def test_update_modifies_file(tmp_path: Path) -> None:
    store = TicketStore(tmp_path / "tickets")
    ticket = make_ticket("CODE-1", title="Needs fix", status=Status.OPEN)
    store._create(ticket)

    updated = store.update("CODE-1", TicketUpdate(status=Status.DONE))
...
def test_delete_removes_file(tmp_path: Path) -> None:
    store = TicketStore(tmp_path / "tickets")
    ticket = make_ticket("CODE-1", title="Delete me")
    path = ticket_path(store.base_dir, ticket)
    store._create(ticket)

    store.delete("CODE-1", force=True)
...
def test_concurrent_id_generation(tmp_path: Path) -> None:
    store = TicketStore(tmp_path / "tickets")
    store._create(make_ticket("CODE-1", title="Existing cleanup"))

    first = store._next_id("code_quality")
    store._create(make_ticket(first, title="Queued cleanup"))
    second = store._next_id("code_quality")
...
def test_create_ticket_is_atomic_under_concurrency(tmp_path: Path) -> None:
    ...
    assert sorted(ids) == [f"CODE-{i}" for i in range(1, 9)]
    assert len({*ids}) == 8
...
def test_next_id_starts_at_1(tmp_path: Path) -> None:
    ...
    assert store._next_id("code_quality") == "CODE-1"
...
def test_count_counts_markdown_files_without_parsing(tmp_path: Path) -> None:
    ...
    (ticket_dir / "CODE-1-valid.md").write_text("---\nid: CODE-1\n---\n", encoding="utf-8")
    (ticket_dir / "CODE-2-invalid.md").write_text("not frontmatter", encoding="utf-8")
...
def test_next_id_uses_filename_scan_without_parsing(tmp_path: Path) -> None:
    ...
    ticket_dir = store.base_dir / "owner" / "repo" / "code-quality"
    ticket_dir.mkdir(parents=True)
    (ticket_dir / "CODE-2-valid.md").write_text("---\nid: CODE-2\n---\n", encoding="utf-8")
    (ticket_dir / "CODE-10-invalid.md").write_text("not frontmatter", encoding="utf-8")
    (ticket_dir / "SEC-99-security.md").write_text("not frontmatter", encoding="utf-8")

    assert store._next_id("code_quality") == "CODE-11"


def test_create_uses_expected_nested_path(tmp_path: Path) -> None:
    store = TicketStore(tmp_path / "tickets")
    ticket = make_ticket("SEC-1", title="Nested path", repo="acme/platform", category="security")

    store._create(ticket)

    assert ticket_path(store.base_dir, ticket) == (
        tmp_path / "tickets" / "acme" / "platform" / "security" / "SEC-1-nested-path.md"
    )
...
def test_create_without_fix_omits_fix_section(tmp_path: Path) -> None:
    store = TicketStore(tmp_path / "tickets")
    ticket = make_ticket("CODE-1", title="Plain body", description="Only description text.")
...
def test_get_is_case_insensitive_for_id_lookup(tmp_path: Path) -> None:
    store = TicketStore(tmp_path / "tickets")
    ticket = make_ticket("CODE-1", title="Lookup")
    store._create(ticket)

    loaded = store.get("code-1")

    assert loaded.id == "CODE-1"


def test_list_returns_sorted_by_ticket_id(tmp_path: Path) -> None:
    store = TicketStore(tmp_path / "tickets")
    store._create(make_ticket("SEC-2", title="Second security", category="security"))
    store._create(make_ticket("CODE-3", title="Third code"))
    store._create(make_ticket("CODE-1", title="First code"))

    results = store.list()

    assert [ticket.id for ticket in results] == ["CODE-1", "CODE-3", "SEC-2"]


def test_update_can_change_description_and_fix(tmp_path: Path) -> None:
    store = TicketStore(tmp_path / "tickets")
    ticket = make_ticket("CODE-1", title="Needs more detail", description="Old", fix=None)
    store._create(ticket)
...
def test_update_ticket_due_date(tmp_path: Path) -> None:
    ...
    store = TicketStore(tmp_path / "tickets")
    ticket = make_ticket("CODE-1", title="Update due")
    store._create(ticket)

    updated = store.update("CODE-1", TicketUpdate(due_date=date(2026, 8, 1)))
...
def test_update_ticket_clear_due_date(tmp_path: Path) -> None:
    ...
    store = TicketStore(tmp_path / "tickets")
    ticket = make_ticket("CODE-1", title="Clear due", due_date=date(2026, 5, 1))
    store._create(ticket)

    updated = store.update("CODE-1", TicketUpdate(due_date=None))
...
def test_update_can_change_category_and_move_file(tmp_path: Path) -> None:
    store = TicketStore(tmp_path / "tickets")
    ticket = make_ticket("CODE-1", title="Reclassify ticket", category="code_quality")
    old_path = ticket_path(store.base_dir, ticket)
    store._create(ticket)

    updated = store.update("CODE-1", TicketUpdate(category="security"))
...
def test_update_title_recomputes_slug_and_renames_file(tmp_path: Path) -> None:
    store = TicketStore(tmp_path / "tickets")
    ticket = make_ticket("CODE-1", title="Original title")
    old_path = ticket_path(store.base_dir, ticket)
    store._create(ticket)

    updated = store.update("CODE-1", TicketUpdate(title="Updated auth title"))
    new_path = ticket_path(store.base_dir, updated)
...
def test_update_is_atomic(tmp_path: Path) -> None:
    store = TicketStore(tmp_path / "tickets")
    ticket = make_ticket("CODE-1", title="Original title", category="code_quality")
    old_path = ticket_path(store.base_dir, ticket)
    store._create(ticket)
...
def test_delete_not_found_raises(tmp_path: Path) -> None:
    ...
    with pytest.raises(TicketNotFoundError):
        store.delete("CODE-404")


def test_create_duplicate_id_raises(tmp_path: Path) -> None:
    store = TicketStore(tmp_path / "tickets")
    ticket = make_ticket("CODE-1", title="Duplicate")
    store._create(ticket)

    with pytest.raises(TicketAlreadyExistsError, match="Ticket CODE-1 already exists"):
        store._create(ticket)
...
def test_list_filters_by_repo_status_and_tags(tmp_path: Path) -> None:
    ...
    store._create(make_ticket("CODE-2", title="Wrong repo", repo="other/app", status=Status.ACTIVE, tags=["auth", "api"]))
    store._create(make_ticket("CODE-3", title="Wrong status", repo="acme/app", status=Status.OPEN, tags=["auth", "api"]))
    store._create(make_ticket("CODE-4", title="Missing tag", repo="acme/app", status=Status.ACTIVE, tags=["auth"]))
    ...
    assert [ticket.id for ticket in results] == ["CODE-1"]


def test_list_filters_repo_wildcards(tmp_path: Path) -> None:
    store = TicketStore(tmp_path / "tickets")
    store._create(make_ticket("CODE-1", title="Match wildcard", repo="ejacklab/core"))
    store._create(make_ticket("CODE-2", title="No match", repo="other/core"))
...
def test_list_filters_by_has_fix_and_owner(tmp_path: Path) -> None:
    store = TicketStore(tmp_path / "tickets")
    store._create(make_ticket("CODE-1", title="Owned fix", owner="smoke01", description="Has a fix", fix="Do it"))
    store._create(make_ticket("CODE-2", title="Owned no fix", owner="smoke01", description="No fix", fix=None))
    store._create(make_ticket("CODE-3", title="Other owner", owner="alex", description="Other owner", fix="Do it"))
...
def test_list_skips_corrupt_files_and_collects_errors(tmp_path: Path) -> None:
    ...
    broken_path = (
        store.base_dir / "acme" / "app" / "code_quality" / "CODE-2-broken-ticket.md"
    )
    ...
    assert store.last_list_errors[0].field == "acme/app/code_quality/CODE-2-broken-ticket.md"


def test_soft_delete_moves_file_to_trash(tmp_path: Path) -> None:
    store = TicketStore(tmp_path / "tickets")
    ticket = make_ticket("CODE-1", title="Trash me", repo="acme/app")
    active_path = ticket_path(store.base_dir, ticket)
    store._create(ticket)

    trash_path = store.move_to_trash("CODE-1")

    assert not active_path.exists()
    assert trash_path == store.base_dir / TRASH_DIRNAME / "acme" / "app" / "code_quality" / "CODE-1-trash-me.md"
    assert trash_path.exists()


def test_soft_delete_creates_trash_directory(tmp_path: Path) -> None:
    store = TicketStore(tmp_path / "tickets")
    ticket = make_ticket("CODE-1", title="Create trash dir")
    store._create(ticket)
...
def test_force_delete_permanently_removes(tmp_path: Path) -> None:
    store = TicketStore(tmp_path / "tickets")
    ticket = make_ticket("CODE-1", title="Hard delete")
    store._create(ticket)
...
def test_restore_from_trash(tmp_path: Path) -> None:
    store = TicketStore(tmp_path / "tickets")
    ticket = make_ticket("SEC-1", title="Restore me", category="security", repo="acme/app")
    original_path = ticket_path(store.base_dir, ticket)
    store._create(ticket)
    store.delete("SEC-1")

    restored = store.restore_from_trash("SEC-1")

    assert restored == ticket
    assert original_path.exists()
    assert not any((store.base_dir / TRASH_DIRNAME).rglob("SEC-1-restore-me.md"))


def test_restore_nonexistent_ticket_raises(tmp_path: Path) -> None:
    store = TicketStore(tmp_path / "tickets")

    with pytest.raises(TicketNotFoundError, match="Ticket CODE-404 not found"):
        store.restore_from_trash("CODE-404")


def test_restore_duplicate_target_raises(tmp_path: Path) -> None:
    store = TicketStore(tmp_path / "tickets")
    original = make_ticket("CODE-1", title="Original title", repo="acme/app")
    store._create(original)
    store.delete("CODE-1")
    store._create(make_ticket("CODE-1", title="Original title", repo="acme/app"))

    with pytest.raises(TicketAlreadyExistsError, match="Ticket CODE-1 already exists"):
        store.restore_from_trash("CODE-1")


def test_list_excludes_trash_directory(tmp_path: Path) -> None:
    store = TicketStore(tmp_path / "tickets")
    active = make_ticket("CODE-1", title="Active ticket")
    trashed = make_ticket("CODE-2", title="Trashed ticket")
...
def test_find_ticket_excludes_trash_directory(tmp_path: Path) -> None:
    store = TicketStore(tmp_path / "tickets")
    ticket = make_ticket("CODE-1", title="Hidden in trash")
    store._create(ticket)
    store.delete("CODE-1")
...
def test_list_supports_sort_by_severity(tmp_path: Path) -> None:
    store = TicketStore(tmp_path / "tickets")
    store._create(make_ticket("CODE-1", title="Medium priority", severity=Severity.MEDIUM))
    store._create(make_ticket("CODE-2", title="Critical priority", severity=Severity.CRITICAL))
    store._create(make_ticket("CODE-3", title="Low priority", severity=Severity.LOW))

    results = store.list(sort_by="severity")

    assert [ticket.id for ticket in results] == ["CODE-2", "CODE-1", "CODE-3"]


def test_list_supports_descending_created_at_sort(tmp_path: Path) -> None:
    store = TicketStore(tmp_path / "tickets")
    older = make_ticket("CODE-1", title="Older")
    newer = make_ticket("CODE-2", title="Newer")
    ...
    assert [ticket.id for ticket in results] == ["CODE-2", "CODE-1"]


def test_list_supports_sort_by_due_date(tmp_path: Path) -> None:
    store = TicketStore(tmp_path / "tickets")
    store._create(make_ticket("CODE-1", title="Due Jun", due_date=date(2026, 6, 15)))
    store._create(make_ticket("CODE-2", title="No due date"))
    store._create(make_ticket("CODE-3", title="Due Jan", due_date=date(2026, 1, 15)))

    results = store.list(sort_by="due_date")

    assert [ticket.id for ticket in results] == ["CODE-3", "CODE-1", "CODE-2"]


def test_list_with_date_filters(tmp_path: Path) -> None:
    store = TicketStore(tmp_path / "tickets")
    older = make_ticket("CODE-1", title="Old ticket")
    newer = make_ticket("CODE-2", title="New ticket")
    between = make_ticket("CODE-3", title="Between ticket")
    ...
    assert sorted([t.id for t in results]) == ["CODE-2", "CODE-3"]

    # created_before
    results = store.list(SearchFilters(created_before=datetime(2026, 4, 1, 0, 0, 0, tzinfo=UTC)))
    assert sorted([t.id for t in results]) == ["CODE-1", "CODE-3"]
...
def test_list_filters_by_due_date(tmp_path: Path) -> None:
    ...
    store._create(make_ticket("CODE-2", title="Due Jun", due_date=date(2026, 6, 15)))
    store._create(make_ticket("CODE-3", title="Due Dec", due_date=date(2026, 12, 15)))

    results = store.list(SearchFilters(due_after=date(2026, 6, 1)))
    assert sorted([t.id for t in results]) == ["CODE-2", "CODE-3"]

    results = store.list(SearchFilters(due_before=date(2026, 6, 1)))
    assert sorted([t.id for t in results]) == ["CODE-1"]
...
def test_list_filters_tickets_without_due_date_excluded_from_range(
    ...
    store = TicketStore(tmp_path / "tickets")
    store._create(make_ticket("CODE-1", title="No due date"))
    store._create(make_ticket("CODE-2", title="Has due", due_date=date(2026, 6, 1)))
...
def test_list_filters_by_severity(tmp_path: Path) -> None:
    store = TicketStore(tmp_path / "tickets")
    store._create(make_ticket("CODE-1", title="Critical", severity=Severity.CRITICAL))
    store._create(make_ticket("CODE-2", title="High", severity=Severity.HIGH))
    store._create(make_ticket("CODE-3", title="Medium", severity=Severity.MEDIUM))
    store._create(make_ticket("CODE-4", title="Low", severity=Severity.LOW))

    # Single severity
    results = store.list(SearchFilters(severity=[Severity.CRITICAL]))
    assert [t.id for t in results] == ["CODE-1"]

    # Multiple severities (OR)
    results = store.list(SearchFilters(severity=[Severity.CRITICAL, Severity.LOW]))
    assert [t.id for t in results] == ["CODE-1", "CODE-4"]
...
def test_update_changing_repo_moves_file(tmp_path: Path) -> None:
    ...
    store = TicketStore(tmp_path / "tickets")
    ticket = make_ticket("CODE-1", title="Move me", repo="old/repo")
    old_path = ticket_path(store.base_dir, ticket)
    store._create(ticket)
    ...
    updated = store.update("CODE-1", TicketUpdate(title="Moved title"))
    new_path = ticket_path(store.base_dir, updated)
...
def test_update_with_empty_update_is_noop(tmp_path: Path) -> None:
    ...
    updated = store.update("CODE-1", TicketUpdate())
